...
const FormAddDataKehadiran = () => {
  ...
  const getDataPegawai = async () => {
    const response = await axios.get("http://localhost:3000/data_pegawai");
    setDataPegawai(response.data);
  };

  const getDataKehadiran = async () => {
    try {
      const response = await axios.get("http://localhost:3000/data_kehadiran");
  ...
  const saveDataKehadiran = async (e) => {
    ...
    try {
      for (let i = 0; i < dataPegawai.length; i++) {
        ...
        if (!isNamaAda) {
          await axios.post("http://localhost:3000/data_kehadiran", {
    ...
    <Layout>
      ...
      <div className="mt-6 rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <form onSubmit={saveDataKehadiran}>
              ...
              <tbody>
                {filteredDataPegawai
                  ...
                  .map((data, index) => {
                    const isNamaAda = dataKehadiran.some(
                      (kehadiran) =>
                        kehadiran.nama_pegawai === data.nama_pegawai,
                    );
                      ...
                      return (
                        <tr
                          ...
                          <td className="px-4 py-5" colSpan="8">
                            <p className="text-center text-black dark:text-white">
                              Data Kehadiran Pegawai Sudah di Simpan. Input
                              Kembali Ketika Sudah Ganti Periode !
                            </p>
                          </td>
                        </tr>
                      );
          ...
          <div className="mt-4 flex flex-col items-center justify-between md:flex-row md:justify-between">
            <div className="flex items-center space-x-2">
              <span className="text-gray-5 dark:text-gray-4 py-4 text-sm">
                ...
                {filteredDataPegawai.length} Data Kehadiran Pegawai
              </span>
            </div>
            <div className="flex space-x-2 py-4">
              <button
                ...
                className="rounded-lg border border-primary px-6 py-2 font-semibold text-primary hover:bg-primary hover:text-white disabled:cursor-not-allowed disabled:opacity-50 dark:border-primary dark:text-white dark:hover:bg-primary dark:hover:text-white">
                <MdKeyboardDoubleArrowLeft />
              </button>
              {paginationItems()}
              <button
          ...
          <div className="flex w-full flex-col gap-3 py-4 text-center md:flex-row">
            <div>
