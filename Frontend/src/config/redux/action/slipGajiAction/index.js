  ...
  (selectedYear, onDataFound) => async (dispatch) => {
    try {
      const response = await axios.get(
        `http://localhost:3000/laporan/slip_gaji/year/${selectedYear}`,
      );
...
export const fetchSlipGajiByMonth =
  (selectedMonth, onDataFound) => async (dispatch) => {
    try {
      const response = await axios.get(
        `http://localhost:3000/laporan/slip_gaji/month/${selectedMonth}`,
      );
      const data = response.data;
      dispatch(fetchSlipGajiSuccess(data));
      onDataFound();
...
export const fetchSlipGajiByName =
  (selectedName, onDataFound) => async (dispatch) => {
    try {
      const response = await axios.get(
        `http://localhost:3000/laporan/slip_gaji/name/${selectedName}`,
      );
      const data = response.data;
      dispatch(fetchSlipGajiSuccess(data));
      onDataFound();
