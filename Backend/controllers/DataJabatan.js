...
export const getDataJabatan = async (req, res) => {
  try {
    ...
    if (req.hak_akses === "admin") {
      ...
      if (req.userId !== DataJabatan.userId)
...
export const getDataJabatanByID = async (req, res) => {
  try {
    ...
    if (response) {
      ...
      res
        .status(404)
        .json({ msg: "Data jabatan dengan ID tersebut tidak ditemukan" });
    }
  } catch (error) {
    res.status(500).json({ msg: error.message });
  }
...
export const createDataJabatan = async (req, res) => {
  const { id_jabatan, nama_jabatan, gaji_pokok, tj_transport, uang_makan } =
    req.body;
  if (isNaN(gaji_pokok) || isNaN(tj_transport) || isNaN(uang_makan)) {
    return res.status(400).json({
      success: false,
      message: "Salary values must be numeric",
    });
  }

  if (
    Number(gaji_pokok) < 0 ||
    Number(tj_transport) < 0 ||
    Number(uang_makan) < 0
  ) {
    return res.status(400).json({
      success: false,
  ...
  try {
    ...
    res
      .status(201)
      .json({ success: true, message: "Data Jabatan Berhasil di Simpan" });
  } catch (error) {
